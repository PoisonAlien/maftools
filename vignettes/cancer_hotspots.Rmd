---
title: "Personlaized somatic report"
...
output: 
  html_document:
    ...
    highlight: pygments
vignette: >
  %\VignetteIndexEntry{03: Somatic report}
...
```{r, eval=FALSE}
can_hs_tbl = maftools::cancerhotspots(bam = "Tumor.bam", refbuild = "GRCh37", mapq = 10, sam_flag = 1024)
```
...
`ntcounts` function extracts nucleotide counts for targeted variants from the BAM file. It is functionally similar to [bam-readcount](https://github.com/genome/bam-readcount) utility and supports multiple BAM files and INDELS.

```{r}
#Generate a sample loci file - first two columns must contain chromsome name and position (1-based coordinate)
loci = data.table::data.table(chr = c("seq1", "seq2"), pos = c(1340, 1483))
loci
loci_file = tempfile()
data.table::fwrite(x = loci, file = loci_file, sep = "\t")
```

```{r}
bamfile = system.file("extdata", "ex1.bam", package = "Rsamtools") #BAM file
loci_rc = maftools::ntcounts(bam = bamfile, loci = loci_file) 
#nucleotide counts
loci_rc
```
