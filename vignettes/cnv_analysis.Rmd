...
`maftools` provides set of functions to facilitate copy number analysis using [ASCAT](https://github.com/VanLoo-lab/ascat) for tumor-normal or tumor only WGS datasets.
Although there exists [ascatNgs](https://github.com/cancerit/ascatNgs), it requires installation of perl and C modules to fetch the readcounts across the markers. `maftools` bypasses these requirements entirely within R with the C code baked in. However, `maftools` only generates the required readcounts, BAF and logR files. Downstream analyses has to be done with [ASCAT](https://github.com/VanLoo-lab/ascat). 
...
If you use the functions for CNV analysis, please cite the ASCAT publication
...
Below command takes `tumor` and `normal_nucleotide_counts.tsv` files, filter SNPs with low coverage (default <15), estimate BAF, logR, and generates the input files for ASCAT.
...
Returned `ASCAT` object can be passed to downstream ASCAT functions. Below code chunk shows a minimal usage with ASCAT. See [here](https://github.com/VanLoo-lab/ascat/tree/master/ExampleData) for further workflow examples. 
...
In tumor only mode, readcounts are normalized for median depth of coverage across autosomes. 
...
Alternatively, tumor logR files generated by `prep_ascat()` can be processed with `segmentLogR()` function that performs circular binary segmentation using [DNAcopy](https://bioconductor.org/packages/release/bioc/html/DNAcopy.html).
...
The output can be imported and analyzed with `maftools` in tumor/normal or tumor only mode.
...
<p align="left">
<img src="https://user-images.githubusercontent.com/8164062/160853005-4f79ae83-8fb1-493c-b240-2b794274c36d.png" height="320" width="700">
</p>
