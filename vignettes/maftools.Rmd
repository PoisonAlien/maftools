...
Once decomposed, signatures are compared against known and validated signatures from Sanger [11](#references). In the above exaple, 2 signatures are derived. One of the signatures is most similar to validated signature_1A with a high correlation coefficient. Signature_1A is a result of elevated rate of spontaneous deamination of 5-methyl-cytosine, which results in C>T transitions and which predominantly occurs at NpCpG trinucleotide which is a most common process in AML [13](#references).
...
Note that by default Simple Somatic Mutation format contains all affected transcripts of a variant resuting in multiple entries of the same variant in same sample. It is hard to choose a single affected transcript based on annotations alone and by default this program removes repeated variants as duplicated entries. If you wish to keep all of them, set `removeDuplicatedVariants` to FALSE. Another option is to convert input file to MAF without removing anything and then use scripts like [vcf2maf](https://github.com/mskcc/vcf2maf) to prioritize affected transcripts.
...
subsetMaf(maf = laml, query = "t_vaf > 30", fields = 'Protein_Change')
```
