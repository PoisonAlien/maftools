...
1. Estimates APOBEC enrichment scores
2. Prepares a mutational matrix for signature analysis.

###APOBEC Enrichment estimation.
Note that above command prints APOBEC Enrichment statistics. APOBEC induced mutations are more frequent in solid tumors and are mainly associated with C>T transition events occuring in TCW motif. This contribution of APOBEC induced mutations can be estimated as an enrichment score. APOBEC enrichment scores in the above command are estimated using the method described by Roberts et al[12](#references). Briefly, enrichment of C>T mutations occuring within TCW motif over all of the C>T mutations in a given sample is compared to background Cytosines and TCWs occuring within 20bp of mutated bases. A one-sided fishers exact test is also performed to statsitsically evaluate the enrichment score, as described in original study by Roberts et al.

                                              E = (n_tcw * background_c) / (n_C * background_tcw)
                                              
were, n_tcw = number of mutations within T[C>T]W and T[C>G]W context. (W -> A or T)
...
12. Roberts SA, Lawrence MS, Klimczak LJ, et al. An APOBEC Cytidine Deaminase Mutagenesis Pattern is Widespread in Human Cancers. Nature genetics. 2013;45(9):970-976. doi:10.1038/ng.2702.
