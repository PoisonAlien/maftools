...
<img src="maftools.svg" height="155" width="250" style="position:absolute;top:30px;right:0px;" />
...
##Required input files
  ...
  * an optional but recommended annotation data containing sample details. Usually clinical data.
...
Also note that using `sortByAnnotation = TRUE` further sorts oncoplot according to annotations provided (here FAB_classification) and helps in clear representation of mutational patters. For example from below plots shows NPM1 is rarely mutated in M0 type of AML wheras RUNX1 is virtually absent in M5 subtype.
...
###oncoplot
This is similar to oncoplots except for CN.
...
laml.dnmt3a.surv = mafSurvival(maf = laml, genes = 'DNMT3A', time = 'days_to_last_followup', Status = 'Overall_Survival_Status')
```
...
```{r, results='hide', message=F, fig.align='left',fig.width=7, fig.height=5, eval = T}
...
####Plotting differences betweeen APOBEC enriched and non-enriched samples

We can also see the mutational pattern differnces between APOBEC enriched and non-APOBEC enriched samples. However LAML is not an ideal cohort for this sort of analysis and hence below plot is only for demonstration purposes. 
...
#Found Signature_1 most similar to validated Signature_1. Aetiology: spontaneous deamination of 5-methylcytosine [cosine-similarity: 0.939] 
#Found Signature_2 most similar to validated Signature_1. Aetiology: spontaneous deamination of 5-methylcytosine [cosine-similarity: 0.888] 
```
