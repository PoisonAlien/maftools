...
##Reading MAF files.
`read.maf` function reads MAF files, summarizes it in various ways and stores it as an MAF object. Even though MAF file is alone enough, it is highly recommended to provide annotations associated with samples in MAF. One can also integrate copy number data if available.
...
laml.mutsig <- system.file("extdata", "LAML_sig_genes.txt.gz", package = "maftools")
laml.pancan = pancanComparison(mutsigResults = laml.mutsig, qval = 0.1, cohortName = 'LAML', inputSampleSize = 200, label = 1, normSampleSize = TRUE)
```
...
`clinicalEnrichment` is another function which takes any clinical feature associated with the samples and performs enrichment analysis. It performs various groupwise and pairwise comparisions to see enriched mutations for every factor within a clincila feature. For example, below are the results using FAB_classification as a clincial feature.
...
plotSignatures(laml.sign, title_size = 1)
```
...
###Plotting Signature contributions
`plotSignatures` also plots signature contribution as a barplot. 

```{r, fig.width=5, fig.height=4, fig.align='center'}
plotSignatures(nmfRes = laml.sign, contributions = TRUE, title_size = 1)
```
...
#Note: pvalue < 0.5 is only plotting purpose since AML has no interesting differences in signatures.
