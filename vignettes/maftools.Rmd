...
Every cancer is driven by a speicifc set of driver genes. We can compare two different cohorts to detect such differentally mutated (driver) genes. For example, recent article by [Madan et. al](http://www.ncbi.nlm.nih.gov/pubmed/27063598) [5](references), have shown relapse specific mutations in PML and RARA genes which were absent during primary stage of the Acute Promyelocytic Leukemia. This difference can be detected using function `mafComapre` which performs fisher test on all genes between two cohorts to detect differentally mutated genes.
...
##Adding readcounts to MAF file from bam files
`addReadCounts` is wrapper script for bam-readcount programme, which takes MAF file as an input and adds read counts from the corresponding bam file [13](#references). `addReadCounts` assumes bam-readcount is installed and is under path. 
...
13. bam-readcount: https://github.com/genome/bam-readcount
