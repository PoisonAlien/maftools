...
maftools functions can be categorized into mainly Visualization and Analysis modules. Each of these functions and a short description is summarized as shown below. Usage is simple, just read your MAF file with `read.maf` and pass the resulting MAF object to the desired function for plotting or analysis.
...
#We will draw oncoplots for top ten mutated genes. (Removing non-mutated samples from the plot for better visualization)
oncoplot(maf = laml.plus.gistic, top = 10, sortByMutation = TRUE, fontSize = 12)
```

This plot shows frequent deletions in TP53 gene which is located on one of the significanlty deleted locus 17p13.2.

###Changing colors, including MutSig Q-Values and adding annotations to oncoplots.
It is often the case that we include meta data to show sample characterstics such as gender, treatment, etc. We can include such meta data by passing them to `annotation` argument of `oncoplot`. We can also change colors for Variant_Classification by providing a named vector of colors. If MutSigCV results are available, you can pass them via argument `mutsig` which will draw all signififcant genes and includes q-values (-log10 transformed) as a side barplot.
...
oncoplot(maf = laml, annotation = laml.fab.anno, mutsig = laml.mutsig, colors = col, mutsigQval = 0.05)
```
...
```{r, fig.height=5, fig.width=6, eval = T}
