...
Oncoplots can be further improved by including annotations (clinical features) associated with samples, changing colors for variant calssifications and including q-values of significance (either generated from MutSig or similar program). Below plot includes,
...
`Filter` column in the above results indicate whether the identified segment passes the definition of "Kataegis" which are defined as those genomic segments containing six or more consecutive mutations with an average intermutation distance of less than or equal to 1,00 bp [5](#references).
...
gcp = gisticChromPlot(gistic = laml.gistic, markBands = "all")
```
...
Many disease causing genes in cancer are co-occuring or show strong exlcusiveness in their mutation pattern. Such mutually exlcuive or co-occuring set of genes can be detected using `somaticInteractions` function, which performs pair-wise Fisher's Exact test to detect such significant pair of genes. `somaticInteractions` function also uses `cometExactTest` to identify potentially altered gene sets involving >2 two genes  [6](#references).

```{r, message=FALSE}
#We will run mutExclusive on top 10 mutated genes. 
somaticInteractions(maf = laml, top = 25, pvalue = 0.1)
```
...
###Plotting Signature contributions and sample stratification
`plotSignatures` also plots signature contribution as a barplot. 

```{r, fig.width=7, fig.height=4, fig.align='center'}
plotSignatures(nmfRes = laml.sign, contributions = TRUE)
```

Alternatively, you can perform PCA to stratify samples based on signature contribution. 
Once again note that, AML is not an ideal cohort for signature analysis, and this should be interpreted carefully.
