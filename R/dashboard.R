dashboard <- function(maf, color = NULL, rmOutlier = TRUE, log_conv = FALSE, titv.color = NULL, sfs = statFontSize, fontSize = fs, n = 10, donut = pie, rawcount = TRUE, stat = NULL, titleSize = NULL, barcodes = NULL, barcodeSize = NULL) {
  ...
  par(mar = c(3, 9, 3, 1))
  b <- barplot(vc.plot.dat,
    axes = FALSE, horiz = TRUE, col = col[names(vc.plot.dat)], border = NA,
  ...
  axis(
    side = 2, at = b, labels = names(vc.plot.dat), lwd = 1.2, cex.axis = fontSize,
  ...
  if (log_conv) {
    axis(
      side = 2, at = 0, labels = "(log10)", lwd = 1.2, font = 3,
  ...
  par(mar = c(3, 3, 3, 1))
  b <- barplot(vt.plot.dat$sum,
    axes = FALSE, horiz = TRUE, col = vt.cols[1:length(vt.plot.dat$variable)],
    border = NA, xlim = c(0, max(xt))
  )
  abline(v = xt, h = 1:length(b) - 0.25, lty = 2, lwd = 0.3, col = grDevices::adjustcolor(col = "gray70", alpha.f = 0.6))
  axis(
    side = 2, at = b, labels = vt.plot.dat$variable, lwd = 1.2, cex.axis = fontSize,
  ...
  par(mar = c(3, 3, 3, 1))
  b <- barplot(titv.sums$value,
    axes = FALSE, horiz = TRUE, col = titv.color[rownames(titv.sums)],
    border = NA, xlim = c(0, xt[length(xt)])
  )
  axis(
    side = 2, at = b, labels = rownames(titv.sums), lwd = 1.2, cex.axis = fontSize,
    las = 2, line = 0.2, hadj = 0.8, font = 3, tick = FALSE
  )
  axis(side = 1, at = xt, lwd = 1.2, font = 3, las = 2, cex.axis = fontSize * 0.9)
  ...
  if (!rawcount) {
    text(
      x = titv.sums$value + 0.03, y = b, labels = titv.sums$raw_value,
  ...
  axis(
    side = 2, at = as.integer(seq(0, max(colSums(vcs)), length.out = 4)), lwd = 1.2, font = 3, las = 2,
  ...
  b <- boxplot(N ~ Variant_Classification,
    data = vcs.m, xaxt = "n", outline = FALSE, lty = 1, lwd = 1.4, outwex = 0,
  ...
  abline(
    v = 1:length(bcol), h = as.integer(seq(0, max(boxH[, boxStat], na.rm = TRUE), length.out = 4)),
    lty = 2,
  ...
  b <- barplot(gs.dat,
    axes = FALSE, horiz = TRUE, col = col[rownames(gs.dat)], border = NA,
    xlim = c(0, max(xt) + (max(xt) * 0.15)), names.arg = rep("", ncol(gs.dat))
  )
  axis(
    side = 2, at = b, labels = colnames(gs.dat), lwd = 1.2, cex.axis = fontSize,
    las = 2, line = 0.2, hadj = 0.8, font = 3, tick = FALSE
  )
  axis(side = 1, at = xt, lwd = 1.2, font = 3, las = 2, cex.axis = fontSize * 0.9)
  title(main = paste0("Top ", n, "\nmutated genes"), adj = 0, cex.main = titleSize[1], font = 3)
  text(
    x = colSums(gs.dat) + 1, y = b, labels = rev(paste0(gs.load$AlteredSamples, "%")),
    font = 4, col = "black", cex = fontSize * 0.9, adj = 0, xpd = TRUE
  )
  abline(
    h = b, v = xt, lty = 2, lwd = 0.3,
