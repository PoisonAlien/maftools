...
mapMutsToSegs <- function(seg, maf, tsb, build = "hg19") {
  ...
  tsb.dat <- tsb.dat[, .(
    Hugo_Symbol, Chromosome, i.Start_Position, i.End_Position,
...
plotCBS <- function(segData, tsb, build = "hg19", chr.colors = NULL, y_lims = NULL, rect_size = 0.1) {
  ...
  par(mar = c(3, 3, 2, 1))
  plot(NA,
    xlim = c(0, max(seg.spl.transformed$End_Position_updated, na.rm = TRUE)),
  ...
  rect(
    xleft = seg.spl.transformed$Start_Position_updated, xright = seg.spl.transformed$End_Position_updated,
