...
plotApobecDiff = function(tnm, maf, title_size = 1.3){
  ...
  par(bty="n", mgp = c(0.5,0.5,0), mar = c(2,3,4,0) + 0.1, las=1, tcl=-.25, font.main=4, xpd=NA)
  ...
  axis(side = 1, at = c(1, 2), labels = na.omit(sub.tbl[,.N,APOBEC_Enriched])[,paste0('N=', N)],
       las = 1, tick = FALSE, font.axis = 2, cex = 2.5, font = 2, cex.axis = 1.4, lwd = 3)
  axis(side = 2, at = yp, lwd = 3, las = 1, font = 2, cex.axis = 1.4)
  ...
  pie(x = pieDat[APOBEC_Enriched %in% 'yes', value], col = pieCol,
      border="white", radius = 0.95, cex.main=0.6, cex = 1.4, font = 2,
      labels =  pieDat[APOBEC_Enriched %in% 'yes', title], clockwise = TRUE)
  ...
  pie(x = pieDat[APOBEC_Enriched %in% 'no', value], col = pieCol,
      border="white",  radius = 0.95, cex.main = 1.33, cex = 1.4, font = 2,
      labels =  pieDat[APOBEC_Enriched %in% 'no', title], clockwise = TRUE)
  symbols(0,0,circles=.4, inches=FALSE, col="white", bg="white", lty=0, add=TRUE)
  ...
  par(bty="n", mgp = c(0.5,0.5,0), mar = c(7,2.5,0,0) + 0.1, las=1, tcl=-.25, font.main=4, xpd=NA)
  barplot(height = t(mcg.gs), beside = TRUE, las =2, ylab = "", border = NA, font.axis = 2,
          font.lab = 2, col = c('maroon', 'royalblue'),
          ylim = c(0, yl = max(apply(mcg.gs, 2, max))), yaxt = "n", cex.names = 1.3)
  axis(side = 2, at = seq(0, max(apply(mcg.gs, 2, max)), length.out = 4), lwd = 3, font.axis = 2, cex = 1.5,
        label = paste0(round((seq(0, max(apply(mcg.gs, 2, max)), length.out = 4))*100, digits = 2), '%'), cex.axis = 1.4)
