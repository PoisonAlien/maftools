...
#' @details The function will filter SNPs with low coverage (default <15), estimate BAF, logR, and generates the input files for ASCAT. In addition, it will run `ASCAT::ascat.loadData()` and `ASCAT::ascat.plotRawData()` for you and returns the `ASCAT` object that can be further processed with ASCAT functions. Alternatively, logR file can be segmented with \code{\link{segment_logR}}
#' @param t_counts read counts from tumor generated by \code{\link{gtSNPs}}
#' @param n_counts read counts from normal generated by \code{\link{gtSNPs}}
...
#' @param normalize If TRUE, normalizes for library size. Default FALSE
#' @return An \code{\link{ascat.loadData}} object; ascat data structure
#' @seealso \code{\link{gtSNPs}} \code{\link{prep_ascat_t}} \code{\link{segment_logR}}
#' @export
prep_ascat = function(t_counts = NULL, n_counts = NULL, sample_name = NA, min_depth = 15, normalize = FALSE){
