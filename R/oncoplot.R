...
oncoplot = function (maf, top = 20, genes = NULL, mutsig = NULL, mutsigQval = 0.1, drawRowBar = TRUE, drawColBar = TRUE,
  ...
  if (writeMatrix) {
    ...
    write.table(mat, paste0(prefix_name, "_onco_matrix.txt"), sep = "\t", quote = FALSE)
  }
  ...
  anno_column_bar = function(index) {
    ...
    grid::pushViewport(grid::viewport(yscale = c(0, 1000),
                                      xscale = c(0.5, n + 0.5)))
    for (i in seq_along(tb)) {
      if (length(tb[[i]])) {
        y = cumsum(tb[[i]])
    ...
    breaks = grid::grid.pretty(c(0, 1000))
    grid::grid.yaxis(at = breaks, label = breaks, gp = grid::gpar(fontsize = 8))
    grid::upViewport()
  }
