...
#' @details This function by default looks for field "AAChange" in maf file. One can also manually specify field name containing amino acid changes. Protein Domains are retrieved
#' from Uniprot database. Labelling of amino acid changes requires ggrepel package to be installed.
#'
#'
...
#' @param AACol manually specify column name for amino acid changes. Default looks for column 'AAChange'
#' @return ggplot object of the plot, which can be futher modified.
#' @export


lollipopPlot = function(maf, gene = NULL, label = F, AACol = NULL){
  ...
  p = p+geom_segment(data = prot.snp.sumamry, aes(x = pos, xend = pos, y = 0.3, yend = count2-0.03))
