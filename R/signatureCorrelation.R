...
signatureEnrichment <- function(maf, sig_res, minMut = 5, useCNV = FALSE, fn = NULL) {
  ...
  sig.enrich <- clinicalEnrichment(
    maf = maf, clinicalFeature = "Signature",
  ...
  b <- barplot(as.matrix(sig.mean.stat),
    ylim = c(0, 1 + max(sig.sd.stat, na.rm = TRUE)), col = cols,
    axes = FALSE, border = 0.1, xaxt = "n"
  )
  for (i in 1:ncol(sig.sd.stat)) {
    segments(
      x0 = b[i], y0 = cumsum(sig.mean.stat[, i]) - sig.sd.stat[, i],
      x1 = b[i], y1 = cumsum(sig.mean.stat[, i]) + sig.sd.stat[, i],
      lwd = 1.5
    )
  }
  axis(
    side = 2, at = seq(0, 1, 0.25), labels = seq(0, 1, 0.25),
    lwd = 1.2, font.axis = 1, cex = 1.5, font = 1
  )
  mtext(
    text = gsub(pattern = "Signature_", replacement = "", x = colnames(sig.mean.stat)),
    side = 1, at = b, font = 1
  )
  ...
  boxplot(
    at = 1:nrow(sig.mean.stat), N ~ Signature, data = mut.load, xaxt = "n", col = cols,
  ...
  mtext(
    text = c(gsub(pattern = "Signature_", replacement = "", x = names(cols))),
    side = 1, at = 1:nrow(sig.mean.stat), font = 1
  )
  mtext(
    text = c("N:", as.numeric(cf.tbl)),
  ...
  if (!is.null(fn)) {
    write.table(
      x = sig.enrich$pairwise_comparision, file = paste0(fn, "_pairwise_comparision.tsv"), sep = "\t",
      quote = FALSE, row.names = FALSE
    )
    write.table(
      x = sig.enrich$groupwise_comparision, file = paste0(fn, "_groupwise_comparision.tsv"), sep = "\t",
      quote = FALSE, row.names = FALSE
    )
    write.table(
      x = sig.enrich$Signature_Assignment, file = paste0(fn, "_Signature_Assignment.tsv"), sep = "\t",
      quote = FALSE, row.names = FALSE
    )
    write.table(
      x = sig.enrich$mutation_load, file = paste0(fn, "_mutation_load.tsv"), sep = "\t",
      quote = FALSE, row.names = FALSE
    )
  }
