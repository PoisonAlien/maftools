...
lollipopPlot2 <- function(m1, m2, gene = NULL, AACol1 = NULL, AACol2 = NULL, m1_name = NULL, m2_name = NULL, m1_label = NULL, m2_label = NULL, refSeqID = NULL, proteinID = NULL, labPosAngle = 0, labPosSize = 0.9,
  ...
  axis(
    side = 2, at = m1.y.at, labels = m1.y.at.lbl, lwd = 1.2, font = 1, las = 2,
    cex.axis = axisTextSize[2]
  )
  axis(
    side = 2, at = -m2.y.at, labels = m2.y.at.lbl, lwd = 1.2, font = 1, las = 2,
    cex.axis = axisTextSize[2]
  )
  ...
  if (!is.null(m1_label)) {
    if (!is.null(m1.lp)) {
      ...
      if (!is.null(m1_label)) {
        text(
          x = m1_label[, pos2], y = m1_label[, count2 + 0.45], labels = m1_label[, conv],
  ...
  if (!is.null(m2_label)) {
    if (!is.null(m2.lp)) {
      ...
      if (!is.null(m2_label)) {
        text(
          x = m2_label[, pos2], y = -m2_label[, count2 + 0.60], labels = m2_label[, conv],
          font = 1, srt = labPosAngle, cex = labPosSize
        )
      }
    }
  ...
  lep <- legend("topleft",
    legend = names(col), col = col, bty = "n", border = NA,
  ...
  if (!showDomainLabel) {
    ...
    lep <- legend(
      x = x_axp, y = 1, legend = names(domain_cols),
