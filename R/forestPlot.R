...
forestPlot <- function(mafCompareRes, pVal = 0.05, fdr = NULL, color = NULL,
  ...
  lim <- max(abs(c(
    m.sigs$log10OR_high[!is.infinite(m.sigs$log10OR_high)],
    m.sigs$log10OR_low[!is.infinite(m.sigs$log10OR_low)]
  ))) + 1
  ...
  mtext(
    text = m.sigs$Hugo_Symbol, side = 4, line = 0.2, at = 1:nrow(m.sigs),
  ...
  par(mar = c(3, 0, 3, 0))
  plot(rep(0, nrow(m.sigs)), 1:nrow(m.sigs),
    xlim = c(0, 1), axes = FALSE,
    pch = NA, xlab = "", ylab = "", ylim = c(0.5, nrow(m.sigs))
  )
  text(
    x = 0.5, y = 1:nrow(m.sigs), labels = as.numeric(unlist(m.sigs[, 2])),
  ...
  text(
    x = 0.5, y = 1:nrow(m.sigs), labels = as.numeric(unlist(m.sigs[, 3])),
    adj = 0, font = 1, cex = 1.4 * (geneFontSize)
  )
  ...
  text(
    x = 0.5, y = 1:nrow(m.sigs), labels = m.sigs$significance,
    adj = 0, font = 1, cex = 1.4 * (geneFontSize)
  )
  ...
  par(mar = c(0, 0, 0, 0))
  legend(
    x = "top", legend = names(color), lwd = lineWidth,
