...
setdiffMAF <- function(x, y, mafObj = TRUE, refAltMatch = TRUE, ...) {
  ...
  if (!mafObj) {
    ...
    maf_x_unique <- MAF(
      data = maf_x_unique[["nonsyn"]], variants.per.sample = mafSummary$variants.per.sample, variant.type.summary = mafSummary$variant.type.summary,
...
intersectMAF <- function(x, y, refAltMatch = TRUE, mafObj = TRUE, ...) {
  ...
  if (!mafObj) {
    ...
    maf_x_common <- MAF(
      data = maf_x_common[["nonsyn"]], variants.per.sample = mafSummary$variants.per.sample, variant.type.summary = mafSummary$variant.type.summary,
      variant.classification.summary = mafSummary$variant.classification.summary, gene.summary = mafSummary$gene.summary,
