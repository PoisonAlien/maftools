...
#' @param apobecEnrichment If TRUE, estimates APOBEC enrichment scores for each sample in MAF. Default FALSE.
...
extractSignatures = function(mat, apobecEnrichment = FALSE, n = NULL, nTry = 6, plotBestFitRes = FALSE, parallel = NULL){

  if(apobecEnrichment){
    #(TpCpW -> TpKpW)
    tckw = c("T[C>G]A", "T[C>G]T", "T[C>T]A", "T[C>T]T")
    #(VpCpW -> VpKpW)
    vckw = c("A[C>G]A", "A[C>G]T", "A[C>T]A", "A[C>T]T",
             "C[C>G]A", "C[C>G]T", "C[C>T]A", "C[C>T]T",
             "G[C>G]A", "G[C>G]T", "G[C>T]A", "G[C>T]T")
    #TpCpW

    tcw = 2580815
    #VpCpW
    #vcw = c("A[C>T]A", "C[C>T]A", "G[C>T]A",
    #"A[C>T]T", "C[C>T]T", "G[C>T]T")
    vcw = 7873095

    rapo = apply(X = mat, 1, function(x){
      num = sum(x[tckw], na.rm = TRUE)/sum(x[vckw], na.rm = TRUE)
      den = tcw/vcw
      return(num/den)
    })

    #colnames(rapo)[1] = 'Tumor_Sample_Barcode'
    rapo = data.table::as.data.table(rapo, keep.rownames = TRUE)
    rapo[, TCKW := rowSums(mat[,tckw])]
    rapo[, VCKW := rowSums(mat[,vckw])]
    rapo = rapo[order(rapo, decreasing = TRUE)]
    colnames(rapo)[1:2] = c('Tumor_Sample_Barcode', 'APOBEC_Enrichment_Score')
    return(rapo)
  }else{
