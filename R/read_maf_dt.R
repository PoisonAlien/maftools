#' Read MAF files.
#' @description Takes MAF file as input and summarizes it in various ways. Also creates oncomatrix - helpful for visualization.
#'
...
#' @return a list of summarized data tables and oncomatrix.
#' @export
...
read.maf = function(maf, removeSilent = T, useAll = F){
  ...
  if(length(missing.fileds) > 0){
    stop(paste('missing', missing.fileds, 'from MAF')) #stop if any of required.fields are missing
  }
