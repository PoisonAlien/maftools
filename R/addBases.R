...
#'
#' @description Decompose a matrix of mutations classified into 96 substitution classes into \code{n} signatures.
#'
#' @details This function decomposes a non-negative matrix into
...
#' @param n decompose matrix into n signatures. Default NULL. Tries to predict best one by running NMF on a range of values and chooses the one with highest cophenetic correlation coefficient.
#' @param
...
extractSignatures = function(mat, n = NULL){
  ...
  if(length(zeroMutClass)){
    message(paste('Found zero mutations for conversions ', zeroMutClass, sep=''))
  ...
  if(is.null(n)){
    ...
    message(paste('Using ',bestFit, ' as a best-fit rank.', sep=''))
    n = bestFit
  }
