print_mat <- function(maf, genes, removeNonMutated = TRUE, colors = NULL,
  ...
  if (!removeNonMutated) {
    tsb.include <- matrix(
      data = 0, nrow = length(genes),
      ncol = length(tsbs[!tsbs %in% colnames(numMat)])
    )
    tsb.include.char <- matrix(
      data = "", nrow = length(genes),
      ncol = length(tsbs[!tsbs %in% colnames(numMat)])
    )
  ...
  nm[nm == 0] <- NA
  image(
    x = 1:nrow(nm), y = 1:ncol(nm), z = nm, axes = FALSE, xaxt = "n", yaxt = "n",
    xlab = "", ylab = "", col = "white"
  ) # col = "#FC8D62"
  ...
  if (length(complex_events) > 0) {
    for (i in 1:length(complex_events)) {
      ...
      nm_temp[ce_idx] <- 0
      image(
        x = 1:nrow(nm_temp), y = 1:ncol(nm_temp), z = nm_temp, axes = FALSE, xaxt = "n",
  ...
  if (plot2) {
    mtext(
      text = rev(percent_alt), side = 4, at = 1:ncol(nm),
      font = 1, line = 0.4, cex = fontSize, las = 2, adj = 0
    )
    if (showBarcodes) {
      mtext(
        text = rownames(nm), side = 1, las = 2, at = 1:nrow(nm),
    ...
    mtext(
      text = rev(percent_alt), side = 2, at = 1:ncol(nm),
  ...
  if (!is.null(clinicalFeatures)) {
    ...
    image(
      x = 1:nrow(annotation), y = 1:ncol(annotation), z = as.matrix(annotation),
    ...
    for (i in 1:length(names(clini_lvls))) {
      ...
      suppressWarnings(image(
        x = 1:nrow(temp_anno), y = 1:ncol(temp_anno), z = temp_anno,
    ...
    if (plot2) {
      mtext(
        text = colnames(annotation), side = 4,
        font = 1, line = 0.4, cex = fontSize, las = 2, at = 1:ncol(annotation)
      )
    } else {
      mtext(
        text = colnames(annotation), side = 2,
        font = 1, line = 0.4, cex = fontSize, las = 2, at = 1:ncol(annotation)
      )
    }
...
get_m12_annotation_colors <- function(a1 = NULL, a1_cf = NULL,
  ...
  if (length(com_anno) > 0) {
    for (i in 1:length(com_anno)) {
      cf_temp <- com_anno[i]
      com_clini_lvls <- unique(c(
        as.character(unlist(lapply(a1[, cf_temp, drop = FALSE], function(x) unique(as.character(x))))),
