summarizeGistic <- function(gistic) {
  ...
  summary <- data.table::data.table(
    ID = c("Samples", "nGenes", "cytoBands", colnames(vc.cast)[2:ncol(vc.cast)]),
  ...
  return(list(
    summary = summary, cnv.summary = vc.cast,
...
gisticMap <- function(gistic) {
  ...
  oncomat <- data.table::dcast(
    data = dat, formula = Cytoband ~ Tumor_Sample_Barcode, value.var = "Variant_Classification", fill = "",
