...
#' @param pvalue Default 0.05 p-value threshold.
...
#' @param kMax Default 3. maximum gene set size if findPathways is TRUE. This is time consuming for > 3 and might lead R to crash!
#' @examples
...
somaticInteractions = function(maf, top = 25, genes = NULL, pvalue = 0.05, returnAll = FALSE, findPathways = TRUE, kMax = 3){
  ...
  if(returnAll){
    ...
    sigPairs = which(x = 10^-abs(interactions) < pvalue, arr.ind = TRUE)
  }
  ...
  if(nrow(interactions) >= 5){
    interactions[10^-abs(interactions) > pvalue] = 0
    diag(interactions) <- 0
    m <- nrow(interactions)
    n <- ncol(interactions)
    ...
    mtext(side = 2, at = 1:m, text = colnames(interactions), cex = 0.8)
    mtext(side = 3, at = 1:n, text = colnames(interactions), las = 2, line = -2, cex = 0.8)
    ...
    w = arrayInd(which(10^-abs(interactions) < pvalue), rep(m,2))
    points(w, pch=".", col="black")
    w = arrayInd(which(10^-abs(interactions) < .01), rep(m,2))
    points(w, pch="*", col="black")
    ...
    mtext(side=4, at = median(atLims), "log10 (p-value)", las=3, cex = 0.9, line = 3)

    par(xpd=NA)
    text(x=n+2.2, y= max(atLims)+1.2, "Co-occurance", pos=4, cex = 0.9)
    text(x=n+2.2, y = min(atLims)-1.2, "Exclusive", pos=4, cex = 0.9)

    points(x = n+1, y = 0.2*n, pch = "*", cex = 2)
    text(x = n+1, y = 0.2*n, " p < 0.01", pos=4, cex = 0.9)
    points(x = n+1, y = 0.1*n, pch = ".", cex = 2)
    text(x = n+1, y = 0.1*n, paste0("p < ", pvalue), pos=4, cex = 0.9)
  }
