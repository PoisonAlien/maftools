...
tcgaCompare <- function(maf, capture_size = NULL, tcga_capture_size = 50, cohortName = NULL, tcga_cohorts = NULL, primarySite = FALSE, col = c("gray70", "black"), bg_col = c("#EDF8B1", "#2C7FB8"), medianCol = "red", decreasing = FALSE, logscale = TRUE, rm_hyper = FALSE, rm_zero = TRUE) {
  ...
  rect(
    xleft = seq(0, length(plot.dat) - 1, 1), ybottom = min(y_lims), xright = seq(1, length(plot.dat), 1),
  ...
  axis(
    side = 1, at = seq(0.5, length(plot.dat) - 0.5, 1), labels = names(plot.dat),
    las = 2, tick = FALSE, line = -0.8
  )
  axis(
    side = 3, at = seq(0.5, length(plot.dat) - 0.5, 1), labels = unlist(samp_sizes),
  ...
  if (logscale) {
    lapply(seq_len(nrow(tcga.cohort.med)), function(i) {
      segments(
        x0 = i - 1, x1 = i, y0 = tcga.cohort.med[i, Median_Mutations_log10],
    ...
    lapply(seq_len(nrow(tcga.cohort.med)), function(i) {
      segments(
        x0 = i - 1, x1 = i, y0 = tcga.cohort.med[i, Median_Mutations],
