...
#' seg = system.file('extdata', 'TCGA.AB.009.hg19.seg.txt', package = 'maftools')
#' TCGA.AB.3009.clust <- inferHeterogeneity(maf = laml, tsb = 'TCGA.AB.3009', segFile = seg, vafCol = 'i_TumorVAF_WU')
...
plotClusters = function(clusters, tsb = NULL, genes = NULL, showCNvars = FALSE, savePlot = FALSE, width = 6, height = 5, colors = NULL){
  ...
  for(i in 1:length(tsb)){
    ...
    #CN altered regions
    tsb.dat.cn.vars = tsb.dat[cluster == 'CN_altered']
    ...
    if(nrow(tsb.dat.cn.vars) > 0){
      if(showCNvars){
        tsb.dat.dens = tsb.dat.dens+geom_point(data = tsb.dat.cn.vars, aes(y = 0, x = t_vaf), size = 3,alpha = 0.3)
      }
    }
