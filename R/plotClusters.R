...
#' TCGA.AB.3009.clust <- inferHeterogeneity(
#'   maf = laml, tsb = "TCGA-AB-3009",
...
plotClusters <- function(clusters, tsb = NULL, genes = NULL, showCNvars = FALSE, colors = NULL) {
  ...
  for (i in 1:length(tsb)) {
    ...
    for (cl in seq_along(tsb_nclust)) {
      par(mar = c(0, 3, 0, 2))
      boxplot(tsb.dat.spl[[cl]][, t_vaf],
        axes = FALSE, ylim = c(0, 1), horizontal = TRUE,
    ...
    par(mar = c(4, 3, 3, 2))
    plot(NA, NA,
      axes = FALSE, xlab = NA, ylab = NA, xlim = c(0, 1),
    ...
    points(
      y = rep(0, length(tsb.dat[, t_vaf])),
      x = tsb.dat[, t_vaf], pch = 16, col = colors[tsb.dat[, cluster]], cex = 1
    )
    title(main = tsb[i], adj = 0, line = 2)
    title(
      main = paste0("MATH: ", round(unique(tsb.dat[!is.na(MATH)][, MATH]), digits = 3)), cex.main = 1,
    ...
    if (!is.null(genes)) {
      if (genes == "CN_altered") {
        ...
        text(
          x = tsb.dat[, t_vaf], y = 0.1 * max(tsb_dens$y, na.rm = TRUE),
    ...
    legend(
      x = "topright", legend = unique(names(colors[tsb.dat[, cluster]])), bty = "n",
