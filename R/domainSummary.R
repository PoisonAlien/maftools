...
pfamDomains <- function(maf = NULL, AACol = NULL, summarizeBy = "AAPos", top = 5, domainsToLabel = NULL, baseName = NULL, varClass = "nonSyn", width = 5, height = 5, labelSize = 1) {
  ...
  if (summarizeBy == "AAPos") {
    ...
    prot.sum <- data.table::data.table(
      HGNC = prot.sum[, Hugo_Symbol], Start = prot.sum[, pos], End = prot.sum[, pos],
  ...
  par(mar = c(4, 4, 2, 2))
  xx <- bubble_plot(
    plot_dat = domainSum, lab_dat = lab_dat, x_var = "nMuts", y_var = "nGenes",
