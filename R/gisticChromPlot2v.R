...
#'
#' @return LoadedGisticObj object which contains a GISTIC object and a broad table
#' @export
#'
#' @examples
...
yload_gistic = function(gistic_res_dir){
    ...
    lf = lf[lf %>% str_starts('all_lesions.conf_')]
    conf = lf %>% str_sub(18,-5)
    res$conf = conf %>% as.numeric()
...
#'   TRUE
#' @param y_lims default NULL, auto determine the G-score ranges (x axis lims) of the plot.
...
gisticChromPlot2v = function(
    ...
    if (is.null(color)) {
        COLORS = c(Amp = "red", Del = "blue",neutral='gray70')
    }else{
        if (is.null(names(color))){
            ...
            stopifnot(length(intersect(c('Amp','Del','gray70'),names(color)))==3)
            COLORS = color
        }
    }
    ...
    if (symmetric == TRUE){
        ...
        if (ratio.l > ratio.r){
            ...
            y_lims_labels[y_lims_labels <0]  = signif(y_lims_labels[y_lims_labels <0] / ratio.l,2)
    ...
    plot(NA,NA, type = 'l', xlim = ylims, ylim = xlims, axes = FALSE, xlab = NA, ylab = NA)
    # 画组名title（中上外框）
    ...
    # 画下面的刻度轴
    axis(side = 1, at = y_lims, las = 1,pos = 0,labels = y_lims_labels)
    mtext(text = "G-Score", side = 1, line = 0.5, cex = 1.2)
    # 画外框
    rect(xleft = ylims[1],ybottom = xlims[1],xright = ylims[2],ytop = xlims[2])
    # 中间染色体框，和染色体label名字
    rect(xleft = -cytobandOffset, xright = cytobandOffset, ytop = xlims[2] - chr.tbl$start,ybottom = xlims[2] - chr.tbl$end,col = chr.tbl$color)
    text(y = apply(xlims[2] - chr.tbl[, 2:3], 1, mean), x = 0, labels = chr.tbl$chr, cex = cytobandTxtSize, col = c("white", "black"))
    # chr 分界线（灰虚线）
    for (i in 1:(length(chr.tbl$end)-1)){
        lines(y=xlims[2] - rep(chr.tbl$end[i],2),x=ylims,lty = 2, col = grDevices::adjustcolor("gray70", 0.25))
    }
    ...
    #         decreasing = TRUE)][Cytoband %in% mb$Cytoband] %>% dplyr::distinct(Cytoband,ystart, .keep_all = TRUE)
        ...
        cyto_peaks_scores$ylims = ylims[sign(cyto_peaks_scores$ystart) / 2 + 1.5] * 0.9  %>% signif(digits = 2)# set label y pos to range(y_lims)
    ...
    #             End_Position, Variant_Classification, fdr, G_Score)]
        ...
        if (nrow(mut_dat) == 0) {
            ...
            # 加字
            text(y = xlims[2] - mut_dat$anno_Position, x=0.5
                 ,labels = mut_dat$Hugo_Symbol, adj = 2
                 ,cex=mutGenesTxtSize
                 ,pos = 2
                 #,srt=90
                 ,font=3,xpd = TRUE
            ...
            for (i in 1:nrow(mut_dat)){
                p = mut_dat[i,c(Start_Position_updated,Start_Position_updated,anno_Position)]
                lines(y=xlims[2] - p, x = c(1,0.8,0.55), lwd = 0.5, col= mut_dat[i,color])
            }
            # 上下界
    ...
    #             End_Position, Variant_Classification, fdr, G_Score)]
        ...
        # 绘图
        par(mar = c(4, 0.2, 0.2, 0.2)) # c(bottom, left, top, right)
        plot(NA, NA, ylim = c(0, chr.lens.cumsum[length(chr.lens.cumsum)]), xlim = c(0, 1), axes = FALSE, xlab = NA, ylab = NA)
        # 画外框
        # rect(xleft = ylims[1], ybottom = xlims[1], xright = ylims[2],ytop = xlims[2])
        # pos (1=bottom, 2=left, 3=top, 4=right).
        if (nrow(mut_dat) == 0) {
            ...
            for (i in 1:nrow(mut_dat)){
                p = mut_dat[i,c(Start_Position_updated,Start_Position_updated,anno_Position)]
                lines(y=xlims[2] - p, x = c(0,0.2,0.45), lwd = 0.5, col= mut_dat[i,color])
            }
            # 上下界
            #lines(y=rep(xlims[2],3), x = c(0,0.2,0.45), lwd=2,col='blue')
            #lines(y=rep(0,3), x = c(0,0.2,0.45), lwd=2,col='blue')
