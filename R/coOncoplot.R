...
coOncoplot <- function(m1, m2, genes = NULL, m1Name = NULL, m2Name = NULL,
  ...
  if (!is.null(clinicalFeatures1) & !is.null(clinicalFeatures2)) {
    if (is.null(annotationColor1) & is.null(annotationColor2)) {
      m12_annotation_colors <- get_m12_annotation_colors(
        a1 = m1, a1_cf = clinicalFeatures1,
  ...
  nm1 <- print_mat(
    maf = m1, genes = genes, removeNonMutated = removeNonMutated,
  ...
  nm2 <- print_mat(
    maf = m2, genes = genes, removeNonMutated = removeNonMutated,
    test = TRUE, colors = colors
  )
  nm2_ncol <- ncol(nm2[[1]])
  nm2_vc_cols <- nm2[[2]]
  ...
  mtext(
    text = rev(genes), side = 2, adj = 0.5, at = 1:ncol(nm),
