...
#' @param parallel Default 4. Number of cores to use. See note on \code{\link{nmf}} for MAC users.
...
#' @export
estimateSignatures = function(mat, nTry = 6, parallel = 4, pConstant = NULL, plotBestFitRes = FALSE){
  ...
  if(!is.null(parallel)){
    nmfTry = NMF::nmfEstimateRank(mat, seq(2,nTry), method='brunet', nrun=10, seed=123456, .opt = paste0('P', parallel)) #try nmf for a range of values
  }else{
