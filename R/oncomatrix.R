createOncoMatrix <- function(m, g = NULL, chatty = TRUE, add_missing = FALSE) {
  ...
  oncomat <- data.table::dcast(
    data = subMaf[, .(Hugo_Symbol, Variant_Classification, Tumor_Sample_Barcode)], formula = Hugo_Symbol ~ Tumor_Sample_Barcode,
...
bubble_plot <- function(plot_dat, lab_dat = NULL, x_var = NULL, y_var = NULL,
  ...
  x_lims <- as.integer(seq(min(as.numeric(plot_dat$x), na.rm = TRUE),
    max(as.numeric(plot_dat$x), na.rm = TRUE),
    length.out = 4
  ))
  ...
  y_lims <- as.integer(seq(min(as.numeric(plot_dat$y), na.rm = TRUE),
    max(as.numeric(plot_dat$y), na.rm = TRUE),
    length.out = 4
  ))
  ...
  if (return_dat) {
    return(list(
      plots_data = plot_dat, label_cords = lab_dat,
  ...
  suppressWarnings(symbols(
    x = plot_dat$x, y = plot_dat$y, circles = plot_dat$size_z, inches = 0.1, bg = plot_dat$color_var, xlim = x_lims[c(1, 4)],
