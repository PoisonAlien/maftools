...
vafCompare <- function(m1, m2, genes = NULL, top = 5, vafCol1 = NULL, vafCol2 = NULL,
  ...
  if (is.null(genes)) {
    genes <- unique(c(
      as.character(getGeneSummary(x = m1)[1:top, Hugo_Symbol]),
  ...
  for (i in seq_len(length(vafdat))) {
    x <- vafdat[[i]]
    b <- boxplot(t_vaf ~ Cohort,
      data = x, xaxt = "n", boxwex = 0.5, outline = FALSE, lty = 1, lwd = 1, outwex = 0,
      staplewex = 0, ylim = c(0, 1), axes = FALSE, border = cols, horizontal = FALSE, ylab = NA, col = NA, xlim = c(0, 2.5)
    )
    stripchart(t_vaf ~ Cohort,
      vertical = TRUE, data = x,
