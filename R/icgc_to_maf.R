...
icgcSimpleMutationToMAF <- function(icgc, basename = NA, MAFobj = FALSE, clinicalData = NULL, removeDuplicatedVariants = TRUE, addHugoSymbol = FALSE) {
  ...
  maf.vc <- c(
    "Splice_Site", "Splice_Site", "Splice_Site", "Splice_Site",
  ...
  icgc$Variant_Type <- suppressWarnings(as.character(factor(
    x = icgc$mutation_type, levels = c("deletion of <=200bp", "insertion of <=200bp", "single base substitution"),
  ...
  required.fields <- c(
    "gene_affected", "assembly_version", "chromosome", "chromosome_start", "chromosome_end", "chromosome_strand", "Variant_Classification", "Variant_Type",
  ...
  # Make MAF table
  icgc.maf <- data.table::data.table(
    Hugo_Symbol = icgc[, gene_affected], Entrez_Gene_Id = NA, Center = NA, NCBI_Build = icgc[, assembly_version], Chromosome = icgc[, chromosome],
  ...
  if (MAFobj) {
    if (length(unique(icgc.maf[, Tumor_Sample_Barcode])) < 2) {
      ...
      icgc.maf <- MAF(
        data = icgc.maf, variants.per.sample = maf.summary$variants.per.sample, variant.type.summary = maf.summary$variant.type.summary,
